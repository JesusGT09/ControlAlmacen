package com.example.controlalmacen.ui.salidaAdmin
...
import com.example.controlalmacen.adapter.SalidaAdminAdapter
...
class SalidaAdminFragment : Fragment() {
    private var pDialog: ProgressDialog? = null
    private val listAnimation: MutableList<Salida> = ArrayList()
    ...
    private fun CargarData() {
        val url = Baseurl+"ListaSalidas.php"
        showDialog()
    ...
    @SuppressLint("UseRequireInsteadOfGet")
    private fun setupRecyclerView(listAnimation: List<Salida>) {
        val myadapter = SalidaAdminAdapter(activity!!, listAnimation)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.action_add -> {
                val intent = Intent(activity, AddSalidaAdminActivity::class.java)
                intent.putExtra("id", "")
