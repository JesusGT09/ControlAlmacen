package com.example.controlalmacen.ui.slideshow
...
import com.example.controlalmacen.adapter.SalidaAdapter
...
class SlideshowFragment : Fragment() {
    ...
    private fun setupRecyclerView(listAnimation: List<Salida>) {
        val myadapter = SalidaAdapter(activity!!, listAnimation)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.action_add -> {
                val intent = Intent(activity, AddSalidaActivity::class.java)
                intent.putExtra("id", "")
